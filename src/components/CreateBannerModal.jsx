...
export default function CreateBannerModal(props) {
    const [saveModal, setSaveModal] = useState(false);
    const [deleteModal, setDeleteModal] = useState(false);
    const fileRef = useRef(null);
  return (
    <div className='fixed top-0 left-0 right-0 bottom-0'>
      <div className='fixed top-0 left-0 right-0 bottom-0 bg-black/30' onClick={()=>{props.setOpenCreateBannerModal(false)}}></div>
      ...
      <div className='fixed top-1/2 left-1/2 -translate-x-[50%] -translate-y-[50%] flex flex-col gap-[12px] bg-white w-[724px] py-[16px] rounded-[8px] border border-[#CBD5E1]'>
            <div className='flex justify-between px-[16px] py-[8px]'>
                <h1 className='text-[16px] font-HelveticaNeueBold leading-[20px]'>Create banner</h1>
                <button className='text-[24px]' onClick={()=>{props.setOpenCreateBannerModal(false)}}>&times;</button>
            </div>
            ...
            <div className='flex justify-between px-[16px]'>
                <button className='flex-1 text-[#031B89] text-center text-[14px] font-HelveticaNeueMedium leading-[17.5px] px-[16px] py-[12px] rounded-[4px]' onClick={()=>{props.setOpenCreateBannerModal(false)}}>Cancel</button>
                <button className='flex-1 text-white text-center text-[14px] font-HelveticaNeueMedium leading-[17.5px] bg-[#031B89] px-[16px] py-[12px] rounded-[4px]' onClick={()=>{props.setOpenCreateBannerModal(false)}}>Save</button>
            </div>
