...
import { CREATE_COUPON, GET_COUPONS } from "../context/mutation";
...
export default function CouponFlow() {
        ...
        <div className="flex gap-[32px]">
          <div className="flex items-center gap-[8px]">
            ...
            <div className="text-[#7487FF] bg-[#F8FAFC] px-[12px] py-[8px] rounded-3xl">
              <select className="bg-transparent outline-none" name="status" value={filters.status} onChange={handleFilterChange}>
          ...
          <div className="flex items-center gap-[8px]">
            ...
            <div className="text-[#CBD5E1] bg-[#F8FAFC] px-[12px] py-[8px] rounded-3xl">
              <select className="bg-transparent outline-none" name="couponType" value={filters.couponType} onChange={handleFilterChange}>
  ...
  <div className="flex gap-2">
    ...
    />
          ...
          <div>
            {filters.status || filters.couponType || filters.expiryDateRange?.end|| filters.expiryDateRange?.start || filters.couponName ? (
              <Button variant="outlined" color="primary" onClick={handleResetFilters}>Reset</Button>
            ) : null}
          </div>
        </div>
      </div>
      {couponsData?.length === 0 ? (
        <div style={{ display: "flex", justifyContent: "center" }}>
          <p className="mt-[48px] text-[#94A3B8] text-[14px] font-[500] leading-[17.5px]">No Coupons To Show</p>
        </div>
      ) : (
      ...
      {editModal && (
        <>
          <div className="fixed top-0 left-0 w-screen h-screen bg-black/30" onClick={() => setEditModal(false)}></div>
          <div className="fixed top-1/2 left-1/2 flex flex-col gap-[12px] py-[16px] bg-white rounded-[8px] w-[724px] -translate-x-[50%] -translate-y-[50%]">
            <div className="px-[16px] py-[8px] flex justify-between items-center">
              <h1 className="text-[16px] font-[700] leading-[20px]">Add Coupon</h1>
              <div className="cursor-pointer text-[24px] font-[400]" onClick={() => setEditModal(false)}>&times;</div>
            </div>
            <div className="px-[16px] flex flex-col gap-[12px]">
              {/* Coupon Code */}
              <div>
                <p className="text-[#64748B] text-[10px] font-[300] leading-[12.5px] italic mb-[4px]">Coupon Code</p>
                <TextField
              ...
              {/* Coupon Percentage */}
              <div>
                <p className="text-[#64748B] text-[10px] font-[300] leading-[12.5px] italic mb-[4px]">Coupon Percentage</p>
                <TextField
              ...
              {/* Coupon Description */}
              <div>
                <p className="text-[#64748B] text-[10px] font-[300] leading-[12.5px] italic mb-[4px]">Coupon Description</p>
                <TextField
              ...
              {/* Coupon Type */}
              <div>
                <p className="text-[#64748B] text-[10px] font-[300] leading-[12.5px] italic mb-[4px]">Coupon Type</p>
                <input
                  className="outline-none w-full p-[12px] border border-[#64748B] rounded-[4px]"
                  type="text"
                  name="type"
              ...
              {/* Coupon Status */}
              <div>
                <p className="text-[#64748B] text-[10px] font-[300] leading-[12.5px] italic mb-[4px]">Coupon Status</p>
              ...
              {/* Expiry Date */}
              <div>
                <p className="text-[#64748B] text-[10px] font-[300] leading-[12.5px] italic mb-[4px]">Expiry Date</p>
                <TextField
              ...
              {/* Categories */}
              <div>
                <p className="text-[#64748B] text-[10px] font-[300] leading-[12.5px] italic mb-[4px]">Categories</p>
                ...
                {D.map((category) => (
                  <span key={category.id} className="bg-gray-200 px-2 py-1 rounded-md text-sm m-1">{category.text}</span>
                ))}
              </div>
              {/* Save Button */}
              <button
      ...
      {deleteItem && (
        <>
          <div className="fixed top-0 left-0 w-screen h-screen bg-black/30" onClick={() => setDeleteItem(false)}></div>
          <div className="fixed flex flex-col gap-[12px] top-1/2 left-1/2 -translate-x-[50%] -translate-y-[50%] bg-white rounded-[8px] p-[24px]">
            <h1 className="text-[16px] font-[500] leading-[20px]">Delete this {deleteItem}?</h1>
            <p className="text-[12px] font-[400] leading-[15px]">This is a permanent action and cannot be undone.</p>
            <div className="flex">
              <button className="flex-1 text-white bg-[#DC2626] px-[16px] py-[8px] rounded-[4px]" onClick={() => setDeleteItem(false)}>Delete</button>
              <button className="flex-1 px-[16px] py-[8px] rounded-[4px]" onClick={() => setDeleteItem(false)}>Cancel</button>
            </div>
          </div>
        </>
      )}
